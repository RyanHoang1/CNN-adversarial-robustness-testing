...
model.load_state_dict(torch.load('CNN_Base_5_Epoch.pt')) # load model trained on 5 epochs
...
def train(model, loader, testloader, num_epoch, epsilon): # Train the model
    ...
    for i in range(num_epoch):
        ...
        for batch, label in tqdm(loader):
            ...
            model.zero_grad()
            loss.backward()
            adv_batch = pgd_evasion_attack(model, batch, label, epsilon)
...
print('Training on 5 epoch model')
