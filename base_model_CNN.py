...
# Load the dataset and train, test splits
...
# i run using cuda on my local computer
device = "cuda" if torch.cuda.is_available() else "cpu" # Configure device
model = LeNet().to(device)
criterion = nn.CrossEntropyLoss() # Specify the loss layer
optimizer = optim.SGD(model.parameters(), lr=.02)

def train(model, loader, testloader, num_epoch): # Train the model
    ...
    for i in range(num_epoch):
        ...
        for batch, label in tqdm(loader):
            ...
            pred = model(batch) # This will call LeNet.forward()
...
# save 2 models for model trained on 5 epochs and on 15 epochs
if(num_epoch == 5):
    torch.save(model.state_dict(), 'CNN_Base_5_Epoch.pt')
if(num_epoch == 15):
    torch.save(model.state_dict(), 'CNN_Base_15_Epoch.pt')

print('------------------------------------------------------------------------------------------------------------------------------------------------------')
epsilon = 25/255 # perturbation magnitude for attack
