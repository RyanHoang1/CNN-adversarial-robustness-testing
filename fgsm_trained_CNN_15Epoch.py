...
num_epoch = 15
epsilon = 25/255 # epsilon value used in adversarial training (change value accordingly)
...
def train(model, loader, testloader, num_epoch, epsilon): # Train the model
    ...
    for i in range(num_epoch):
        ...
        for batch, label in tqdm(loader):
            ...
            adv_batch = fgsm_evasion_attack(batch, epsilon, data_grad)
...
#pgd evasion attack
def pgd_evasion_attack(model, images, labels, epsilon, steps, step_size=0.01):
...
def eval_pgd_adv_images(model, device, testloader, epsilon, steps):
    #Accuracy Counter
    correct = 0
    # Loop over all examples in test set(in batches of 100)
    for data, label in tqdm(testloader):
        data, label = data.to(device), label.to(device)
        # perform pgd attack
        perturbed_data = pgd_evasion_attack(model, data, label, epsilon, steps)
    ...
    # Calculate final accuracy
    final_acc = correct/(float(len(testloader))*100)
    print("Steps {} - Accuracy = {} / {} = {}".format(steps, correct, (len(testloader)*100), final_acc))
...
eval_pgd_adv_images(model, device, testloader, epsilon, 10)